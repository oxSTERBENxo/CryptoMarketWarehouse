...
interface TopMarketCapTableProps {
...
function Row({ entry, index }: { entry: CurrentMarketCoin; index: number }) {
  ...
  return (
    <tr className="table-row--clickable" {...rowProps}>
      ...
      <td>{entry.market_cap_rank != null ? formatNumber(entry.market_cap_rank) : "—"}</td>
    </tr>
  );
}

export function TopMarketCapTable({ entries, loading, error, onRetry }: TopMarketCapTableProps) {
  const { t } = useTranslation();
  return (
    <section aria-labelledby="top-market-cap-heading" className="table-section">
      <h2 id="top-market-cap-heading">{t("dashboard.topMarketCap.title")}</h2>
      <SectionStatus loading={loading} error={error} isEmpty={!entries || entries.length === 0} onRetry={onRetry}>
        <div className="table-scroll">
          <table>
            <caption className="sr-only">{t("dashboard.topMarketCap.caption")}</caption>
            <thead>
              <tr>
                <th scope="col">#</th>
                ...
                <th scope="col">{t("table.coingeckoRank")}</th>
              </tr>
            </thead>
            <tbody>
